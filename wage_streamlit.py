...
jp_lat_lon = jp_lat_lon.dropna(subset=['pref_name'], axis=0, inplace=False)

##列名変更

jp_lat_lon = jp_lat_lon.rename(columns={'pref_name': '都道府県名'})
# print(jp_lat_lon.shape)
# jp_lat_lon
...
wiew = pdk.ViewState(
    longitude=139.691648,    #東京都の緯度,経度
    latitude=35.689185,
    zoom=4,         #拡大度
    pitch=40.5
)
...
layer = pdk.Layer(
    ...
    threshold = 0.3,
    get_weight = '一人当たり賃金（相対値）' #どの列をヒートマップにするのか
)


layer_map = pdk.Deck(
    layers=layer,
    initial_view_state=wiew
)
...
st.write('■年齢階級別の全国の一人当たり平均賃金（万円）')
...
fig = px.bar(df_mean_categ, 
            ...
            y='産業大分類名',
            color='産業大分類名',   #
            animation_frame='年齢', #年齢階級別なので、年齢
            range_x=[0,max_x],  #0~最大値
            orientation='h',    #横棒グラフの表示の際に設定
            width=800,  #横
            height=500 #縦
)
